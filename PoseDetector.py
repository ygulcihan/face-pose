...
class PoseDetector:
    ...
    def convertToDegrees(self, angles):
        ...
        self.__pitch__ = np.round(angles[0] * 180 * np.pi * self.angle_coefficient)
        self.__yaw__ = np.round(angles[1] * 180 * np.pi * self.angle_coefficient)
    ...
    def start(self):
        ...
        self.__user_image__ = face_recognition.load_image_file("train_img/yigit1.jpg")
        self.__user_face_encoding__ = face_recognition.face_encodings(self.__user_image__)[0]
        ...
        if results.multi_face_landmarks:
            ...
            if self.draw_face_box:    
                ...
                for (top, right, bottom, left), name in zip(self.__face_locations__, self.__face_names__):
                    ...
                    cv2.rectangle(image, (left, top), (right+10, bottom+10), (0, 0, 255), 2)

                    # Draw a label with a name below the face
                    cv2.rectangle(image, (left, bottom - 25), (right+10, bottom+10), (0, 0, 205), cv2.FILLED)
                    font = cv2.FONT_HERSHEY_DUPLEX
                    cv2.putText(image, name, (left + 16, bottom + 4), font, 1.1, (255, 255, 255), 1)
    ...
    def face_recog(self, pimage):
        ...
        if self.__process_this_frame__:
